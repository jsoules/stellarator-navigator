...
// Vendored from https://github.com/magland/figurl-franklab-views/blob/master/test-gui/src/package/util-rate-limiters/rateLimiters.ts
// (since I never got around to writing a proper reusable library for it)
...
/*
  ...
  will resolve all data elements received after the first element and before the resolution fires.
...
export const useDebouncer = <T, TRefs, ResolverProps>(
    ...
    const debouncer = useCallback((state: T) => {
        ...
        if (change) {
            ...
            lastRequest.current = window.setTimeout(resolver, time)
        }
