...
/**
 * Hook to expand a device's coils across the full stellarator ring.
 * 
 ...
 */
export const useFullRingSurface = (inputSurfaceObject: SurfaceApiResponseObject, nfp: number) => {
    const {surfacePoints, pointValues} = inputSurfaceObject
    // Step 1: convert the fields--again, a 30x30 grid of 3-vector points--to matrices.
    // Flatten to 900 
    const surfaceMatrices = useMemo(() => {
        if (surfacePoints === undefined) return []
        return surfacePoints.map((field) => {
            return matrix(field.flat().map(v => [...v]))
        })
    }, [surfacePoints])

    const fullPeriodSurfaceMatrices = useMemo(() => {
        if (surfaceMatrices.length === 0) return [] // data is still loading
    ...
    const completedSurfaces = useMemo(() => {
        if (fullPeriodSurfaceMatrices.length === 0) return []
        // const transforms = surfaceTransformMatrices[nfp]
        const transforms = surfaceTransformMatrices[nfp]
        // const realizedSurfaceMatrices = transforms.map(t => surfaceMatrices.map(obj => multiply(obj, t))).flat()
        // I *think* this should give me a list of matrices for each surface
        const realizedSurfaceMatrices = fullPeriodSurfaceMatrices.map(obj => transforms.map(t => multiply(obj, t)))
        // I *think* we can now unflatten to rows of 30 wide
        // and everything should Just Work
        // --> EXCEPT NO. Because we care about point ordering for the surface triangulation.
        // The upshot is if something is the stellarator-symmetry, we need to reverse each major row order
        // and then prepend it to the original version.
    ...
    return { surfacePoints: completedSurfaces, pointValues: completedPoints }
}
